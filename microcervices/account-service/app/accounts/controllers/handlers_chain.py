...
class AccountAlreadyExistsHandler(DataHandler, AccountHandler):
	def convert_to_response(self, data_to_handle):
		if not data_to_handle:
			return {'message': f'Аккаунт с номером {self.account_number} уже существует.'}


class AccountCreatingHandler(DataHandler, AccountHandler):
	def convert_to_response(self, data_to_handle):
		if data_to_handle:
			return {'message': f'Аккаунт c номером {self.account_number} был успешно создан.'}


class AccountDeletingHandler(DataHandler, AccountHandler):
	def convert_to_response(self, data_to_handle):
		if data_to_handle:
			return {'message': f'Аккаунт с номером {self.account_number} был успешно удален.'}
